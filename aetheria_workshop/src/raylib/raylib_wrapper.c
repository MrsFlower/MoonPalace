...
// Simple string buffer for passing strings from MoonBit
...
void SetStringBuffer_byte(int b) {
    if (string_buffer_len < sizeof(string_buffer) - 1) {
        string_buffer[string_buffer_len++] = (char)b;
        string_buffer[string_buffer_len] = '\0';
    }
}

// Ensure the struct matches MoonBit layout: {id, width, height, mipmaps, format}
// Wasm-GC / Native structs are returned as pointers? No, in MoonBit Native backend, structs might be returned directly or via pointer.
// Since MoonBit native backend ABI passes structs in registers or memory, it's safer to pass it as an Int or allocate it.
// Wait! Returning a C struct `Texture2D` to MoonBit `Texture2D` struct might crash if the ABI doesn't match perfectly.
// Let's use pointers or flat Ints instead.
// Wait, `Texture2D` in raylib is `{ unsigned int id; int width; int height; int mipmaps; int format; }`.
// In MoonBit it's `Int` (32-bit). It should perfectly match.
// Let's try returning it by value. MoonBit C-FFI supports structs.

int64_t LoadTexture_wrapper(int ignored) {
    Texture2D t = LoadTexture(string_buffer);
    Texture2D* ptr = (Texture2D*)malloc(sizeof(Texture2D));
    *ptr = t;
    return (int64_t)(intptr_t)ptr;
}

void DrawTexture_wrapper(int64_t texture_ptr, int posX, int posY, int tint_r, int tint_g, int tint_b, int tint_a) {
    Texture2D* texture = (Texture2D*)(intptr_t)texture_ptr;
    if (!texture) return;
    Color tint = { (unsigned char)tint_r, (unsigned char)tint_g, (unsigned char)tint_b, (unsigned char)tint_a };
    DrawTexture(*texture, posX, posY, tint);
}

void DrawTexturePro_wrapper(int64_t texture_ptr, double src_x, double src_y, double src_w, double src_h, double dst_x, double dst_y, double dst_w, double dst_h) {
    Texture2D* texture = (Texture2D*)(intptr_t)texture_ptr;
    if (!texture) return;
    Rectangle src = { (float)src_x, (float)src_y, (float)src_w, (float)src_h };
    Rectangle dst = { (float)dst_x, (float)dst_y, (float)dst_w, (float)dst_h };
    Vector2 origin = { 0.0f, 0.0f };
    Color tint = { 255, 255, 255, 255 };
    DrawTexturePro(*texture, src, dst, origin, 0.0f, tint);
}

void UnloadTexture_wrapper(int64_t texture_ptr) {
    Texture2D* texture = (Texture2D*)(intptr_t)texture_ptr;
    if (!texture) return;
...
void EnableEventWaiting_wrapper() {
    EnableEventWaiting();
}

void DisableEventWaiting_wrapper() {
    DisableEventWaiting();
}
...
void DrawLine_wrapper(int32_t start_x, int32_t start_y, int32_t end_x, int32_t end_y, int32_t r, int32_t g, int32_t b, int32_t a) {
    Color color = { (unsigned char)r, (unsigned char)g, (unsigned char)b, (unsigned char)a };
...
int32_t IsMouseButtonReleased_wrapper(int32_t button) {
...
double GetTime_wrapper() {
    return GetTime();
}

int GetRandomValue_wrapper(int min, int max) {
